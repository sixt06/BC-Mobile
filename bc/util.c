...
extern void bc_iphone_putchar(int ch);
...
void out_printf (char *str, ...)
{
     ...
     if (!(buf = calloc(1, strlen(str) + 1024))) {
     ...
     for (i=0; i<strlen(buf); i++)
	  bc_iphone_putchar(buf[i]);

     free(buf);
}

void out_vprintf (char *str, va_list args)
{
     char *buf;
     int i;
...
welcome()
{
     out_printf ("This is free software with ABSOLUTELY NO WARRANTY.\n");
     out_printf ("For details type `warranty'. \n");
}
...
limits()
{
     out_printf ("BC_BASE_MAX\t= %d\n",  BC_BASE_MAX);
     out_printf ("BC_DIM_MAX\t\t= %ld\n", (long) BC_DIM_MAX);
     out_printf ("BC_SCALE_MAX\t= %d\n",  BC_SCALE_MAX);
     out_printf ("BC_STRING_MAX\t= %d\n",  BC_STRING_MAX);
     out_printf ("MAX Exponent\t\t= %ld\n", (long) LONG_MAX);
     out_printf ("Number of vars\t\t= %ld\n", (long) MAX_STORE);
#ifdef OLD_EQ_OP
     out_printf ("Old assignment operatiors are valid. (=-, =+, ...)\n");
#endif 
}
...
void yyerror (char *str, ...)
{
     ...
     va_start (args, str);
     if (line_no) 
	  out_printf("Line %d ", line_no);
     out_vprintf (str, args);
     out_printf ("\n");
     had_error = TRUE;
     va_end (args);
}
...
int ignore_warning = 1;
void bc_ignore_warning(int value)
{
     ignore_warning = value;
}
...
#endif
{
  ...
  out_printf ("Runtime error (func=%s, adr=%d): ",
...
#endif
{
  ...
  out_printf ("Runtime warning (func=%s, adr=%d): ",
	      f_names[pc.pc_func], pc.pc_addr);
#ifndef VARARGS   
  va_start (args, mesg);
#else
